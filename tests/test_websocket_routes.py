...
async def test_websocket_creates_audio_bridge():
    ...
    with patch(
        "src.voice_ai_system.api.routes.twilio_refactored.audio_bridge_manager"
    ) as mock_manager:
        # Setup mocks
        ...
        # Connect WebSocket
        with client.websocket_connect("/ws/media/test-workflow") as websocket:
            # Send start event
            websocket.send_json(
                {
                    "event": "start",
...
async def test_periodic_transcript_sync():
    ...
    with patch(
        ...
        with patch(
            "src.voice_ai_system.api.routes.twilio_refactored._sync_transcripts_to_workflow"
        ) as mock_sync:
            mock_session = AsyncMock()
            mock_manager.create_session.return_value = mock_session
...
def test_twiml_generation(test_client):
    ...
    response = test_client.post("/twiml/test-workflow")

    assert response.status_code == 200
    assert "application/xml" in response.headers["content-type"]
    assert "<Stream" in response.text
    assert "ws://testserver/ws/media/test-workflow" in response.text
